...
ref_error_rate = 24.18
ref_compute_time = 330.0
nb_param_ref = 301821
...
def score_eval(load_epoch):
	
	os.system("tail -n 500 out.txt | grep \"Total Net. nb weights: \" | sed \"s/Total Net. nb weights: //g\" > nb_param.txt")
	nb_param = np.loadtxt("nb_param.txt")
	compute_time = np.loadtxt("compute_time.txt")
